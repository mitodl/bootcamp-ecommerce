...
def test_send_batch(settings, sender_name, mock_post):
    ...
    MailgunClient.send_batch('email subject', 'email body', batch_recipient_arg, sender_name=sender_name)
    ...
    assert sorted(called_kwargs['data']['to']) == sorted([email for email, _ in batch_recipient_arg])
...
def test_send_batch_recipient_override(settings, mock_post):
    ...
    MailgunClient.send_batch('subject', 'body', batch_recipient_arg, sender_name='sender')
    assert mock_post.called is True
    called_args, called_kwargs = mock_post.call_args
    assert list(called_args)[0] == '{}/{}'.format(settings.MAILGUN_URL, 'messages')
    assert called_kwargs['auth'] == ('api', settings.MAILGUN_KEY)
...
def test_send_with_sender_address(settings, mock_post):
    ...
    MailgunClient.send_batch(
        'email subject', 'email body', batch_recipient_arg, sender_address=sender_address
    )
    MailgunClient.send_individual_email(
        'email subject', 'email body', individual_recipient_arg, sender_address=sender_address
    )
